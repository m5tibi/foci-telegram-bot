# bot.py (V16.0 - Teljesen Automata Előfizetéssel)
...
# --- Konverziós Állapotok (csak a beszélgetés alapú admin parancsokhoz) ---
...
def subscriber_only(func):
    ...
    async def wrapped(update: telegram.Update, context: CallbackContext, *args, **kwargs):
        ...
        if await is_user_subscribed(user_id):
            ...
            await context.bot.send_message(chat_id=user_id, text="Ez a funkció csak érvényes előfizetéssel érhető el. A /start paranccsal tudsz előfizetni.")
    return wrapped
...
async def start(update: telegram.Update, context: CallbackContext):
    ...
    try:
        await supabase.table("felhasznalok").upsert({"chat_id": user.id, "is_active": True}, on_conflict="chat_id").execute()
...
async def admin_menu(update: telegram.Update, context: CallbackContext):
    keyboard = [[InlineKeyboardButton("👥 Felh. Száma", callback_data="admin_show_users"), InlineKeyboardButton("❤️ Rendszer Státusz", callback_data="admin_check_status")],
                ...
                [InlineKeyboardButton("🔍 Sérültek", callback_data="admin_check_tickets")], [InlineKeyboardButton("🚪 Bezárás", callback_data="admin_close")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Admin Panel:", reply_markup=reply_markup)

# ... (a többi admin funkció, mint `admin_show_users`, stb. aszinkron Supabase hívásokra átírva)
...
async def activate_subscription_and_notify(chat_id: int, app: Application):
    try:
        ...
        await supabase.table("felhasznalok").update({"subscription_status": "active", "subscription_expires_at": expires_at.isoformat()}).eq("chat_id", chat_id).execute()
