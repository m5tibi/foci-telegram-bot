...
async def get_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # --- DIAGNOSZTIKAI LÉPÉS ---
        logger.info("Statisztika lekérdezés indul a Supabase-től...")
        response = supabase.table('tipp_elo_zmenyek').select('*').in_('statusz', ['Nyert', 'Veszített']).execute()
        logger.info(f"Supabase VÁLASZA a statisztika lekérdezésre: {response}") # Kiírjuk a teljes választ a naplóba
        # --- DIAGNOSZTIKA VÉGE ---
