# bot.py (V16.2 - Végleges, Teljes, Javított Verzió)
...
async def start(update: telegram.Update, context: CallbackContext):
    ...
    try:
        def sync_task():
            res = supabase.table("felhasznalok").select("subscription_status").eq("chat_id", user.id).maybe_single().execute()
            if not res.data:
...
def add_handlers(application: Application):
    registration_conv = ConversationHandler(
        ...
        states={ 1: [MessageHandler(filters.TEXT & ~filters.COMMAND, redeem_code)] }, # Itt AWAITING_CODE helyett a számot használjuk
