# bot.py (V6.2 - Kétlépcsős jóváhagyás és Diagnosztika)
...
async def admin_menu(update: telegram.Update, context: CallbackContext):
    keyboard = [
        ...
        # JAVÍTÁS: Új diagnosztikai gomb hozzáadása
...
# === ÚJ DIAGNOSZTIKAI FUNKCIÓ ===
@admin_only
async def test_service_key(update: telegram.Update, context: CallbackContext):
    query = update.callback_query
    await query.answer("Service kulcs ellenőrzése...")
    
    key = SUPABASE_SERVICE_KEY # Beolvassuk a globális változót
    
    if key:
        response_text = (
            f"✅ A `SUPABASE_SERVICE_KEY` be van állítva a Render környezetben.\n\n"
            f"🔑 Kulcs részletei:\n"
            f"  - Első 5 karakter: `{key[:5]}`\n"
            f"  - Utolsó 5 karakter: `{key[-5:]}`\n\n"
            f"Ha a gomb továbbra sem működik, akkor a Renderben elmentett kulcs értéke hibás. Kérlek, másold be újra a helyes kulcsot a Supabase oldaláról és indítsd újra a szolgáltatást."
        )
    else:
        response_text = (
            f"❌ **HIBA:** A `SUPABASE_SERVICE_KEY` **NINCS** beállítva a Render környezeti változói között!\n\n"
            f"Kérlek, add hozzá a kulcsot a Render -> Environment oldalon a hiba elhárításához."
        )
...
async def button_handler(update: telegram.Update, context: CallbackContext):
    ...
    # JAVÍTÁS: Új handler hozzáadása a diagnosztikai gombhoz
...
def add_handlers(application: Application):
    ...
    return application
```Rendben, teljesen megértem a helyzetet. Ha a `SUPABASE_SERVICE_KEY` már be van állítva, de a jóváhagyás mégsem működik, akkor a legvalószínűbb ok az, hogy a Renderbe beírt kulcs **értéke** hibás, még ha a neve (`SUPABASE_SERVICE_KEY`) helyes is.

Ez előfordulhat egy egyszerű másolási hiba miatt (pl. lemarad egy karakter), vagy ha véletlenül egy másik kulcsot (pl. az `anon_key`-t) másoltad be a `service_role_key` helyett.

Hogy ezt egyszer és mindenkorra kizárjuk, beépítettem egy **diagnosztikai funkciót** a botodba. Ez egy új gombot ad az admin menüdhöz, amivel közvetlenül a Render szerverről tudod lekérdezni, hogy a botod látja-e a service kulcsot, és ha igen, mi az első és utolsó 5 karaktere. Így össze tudod hasonlítani azzal, amit a Supabase oldalán látsz.

### Javítás és Diagnosztika

Kérlek, cseréld le a teljes `bot.py` fájlod tartalmát az alábbi, diagnosztikai funkcióval kiegészített verzióra.

---

### `bot.py` (V6.2 - Diagnosztikai Funkcióval)
