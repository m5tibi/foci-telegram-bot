# bot.py (V5.0 - Dátumozott UI és Havi Statisztika)
...
async def tippek(update: telegram.Update, context: CallbackContext):
    ...
    for date_str, tips_on_day in grouped_tips.items():
        # Dátum fejléc generálása magyar napnévvel
...
async def napi_tuti(update: telegram.Update, context: CallbackContext):
    ...
    # Dátum fejléc hozzáadása
    szelveny_date = datetime.fromisoformat(szelveny['created_at']).astimezone(HUNGARY_TZ)
    date_header = f"*--- Napi Tuti - {szelveny_date.strftime('%Y.%m.%d.')} ---*"
    
    message_parts = [date_header]
...
async def stat(update: telegram.Update, context: CallbackContext):
    ...
    # Átváltás UTC-re az adatbázis lekérdezéshez
    ...
    # Fejléc elkészítése magyar hónapnévvel
    month_header = f"*{now.year}. {HUNGARIAN_MONTHS[now.month - 1]}*"

    try:
        # 1. Általános tippek statisztikája az aktuális hónapra
        ...
        # 2. Napi Tuti statisztikája az aktuális hónapra
...
def add_handlers(application: Application):
    ...
    application.add_handler(CommandHandler("tippek", tippek)) # Parancsok továbbra is működhetnek
