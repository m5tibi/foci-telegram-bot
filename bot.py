    ...
    try:
        ...
        for row in records_meccsek:
            if any(row.get(tip_key) not in INVALID_TIPS for tip_key in ['tipp_1x2', 'tipp_goals', 'tipp_btts']):
                ...
                # --- ITT A JAVÍTÁS: A zárójeleket is "escape"-eljük ---
                home_team_safe = home_team.replace("-", "\\-").replace(".", "\\.").replace("(", "\\(").replace(")", "\\)")
                away_team_safe = away_team.replace("-", "\\-").replace(".", "\\.").replace("(", "\\(").replace(")", "\\)")
