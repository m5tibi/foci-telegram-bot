...
def fetch_match_list() -> dict:
    """Mindig refresh-odds-only ELŐBB, aztán match-list lekérés."""
    import time as _time
    headers = {"X-Admin-Password": PERC90_ADMIN_PASS}
    # 1. Refresh: friss meccsek betöltése
    try:
        ...
        print(f"[claude_gen] Refresh hiba: {e}")
    # 2. Match lista lekérése
...
def generate_tips_raw() -> dict:
    ...
    # Debug: első meccs commence értéke
    if all_matches:
        print(f"[claude_gen] Első meccs commence: '{all_matches[0].get('commence','?')}'")
    # Szűrés: mai napnál korábbi meccsek kiszűrése
    from datetime import datetime as _dt2
    import pytz as _ptz2
    _now_bp = _dt2.now(_ptz2.timezone("Europe/Budapest"))
    _today = _now_bp.strftime("%m.%d")  # pl. "08.29"
    _yesterday = (_now_bp - __import__("datetime").timedelta(days=1)).strftime("%m.%d")
    matches = [m for m in all_matches if _yesterday not in str(m.get("commence",""))[:5]]
    if len(matches) < len(all_matches):
        print(f"[claude_gen] {len(all_matches)-len(matches)} tegnapi meccs kiszűrve ({_yesterday}), {len(matches)} maradt")
    if not matches:
        print("[claude_gen] Csak tegnapi meccsek, refresh indul...")
