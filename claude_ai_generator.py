...
Válaszolj KIZÁRÓLAG JSON OBJEKTUMMAL."""
    ...
    json_example = '{"singles":[{"match":"Csapat A vs B","market":"Over 2.5","pick":"Over 2.5","odds":1.78,"note":"Mindkét csapat erős támadójátékkal érkezik.","commence":"08.17 19:00"},{"match":"C vs D","market":"BTTS","pick":"Igen","odds":1.72,"note":"Mindkét csapat betalált az utóbbi 5 meccsén.","commence":"08.17 21:00"}],"combos":[{"legs":[{"match":"X vs Y","pick":"X gyozelem","odds":1.35,"commence":"08.17 19:00"},{"match":"A vs B","pick":"Over 1.5","odds":1.40,"commence":"08.17 21:00"}],"total_odds":1.89,"note":"Indoklás."},{"legs":[{"match":"C vs D","pick":"BTTS Igen","odds":1.30,"commence":"08.17 20:00"},{"match":"E vs F","pick":"E -0.5","odds":1.50,"commence":"08.17 20:30"}],"total_odds":1.95,"note":"Indoklás."}],"free_tip":{"type":"single","match":"X vs Y","market":"Over 2.5","pick":"Over 2.5","odds":1.72,"note":"Indoklás.","commence":"08.17 19:00"},"summary":"Összegzés."}'
...
def save_to_supabase(tips: dict, skip_free: bool = False) -> dict:
    ...
    for t in tips.get("singles", []):
        ...
        for prefix in ["Sajnos", "FIGYELEM", "Hibás", "Újratervezem", "kihagyjuk"]:
            if prefix.lower() in note.lower()[:50]:
                ...
                break
        t["note"] = note
        # tipp_neve előkészítése (piac prefix csak ha nem 1X2 és nem azonos a pick-kel)
        _mkt = t.get('market', '') or ''
        _pck = t.get('pick', '') or ''
        _mkt_prefix = "" if (not _mkt or _mkt.lower() == '1x2' or _mkt.lower() == _pck.lower()) else f"{_mkt}: "
        row = {
            "tipp_neve": f"[AI] {t['match']} – {_mkt_prefix}{_pck} @ {t['odds']}{' 🕐 '+t.get('commence','') if t.get('commence') else ''}",
            "eredo_odds": t["odds"],
    ...
    _fallback_saved: set = set()  # kombi→single fallbacknél már mentett meccsek
    for i, c in enumerate(tips.get("combos", []), 1):
        ...
        if invalid_legs:
            ...
            # Fallback: 1.65+ oddsú lábakat single-ként mentjük
            for leg in invalid_legs:
                leg_odds  = float(leg.get("odds", 0) or 0)
                leg_match = leg.get("match", "")
                if leg_odds < 1.65 or not leg_match:
                    continue
                ...
                if r2.status_code in (200, 201):
                    _fallback_saved.add(leg_match)
                    saved.append(single_row)   # saját dict, nem a Supabase response
