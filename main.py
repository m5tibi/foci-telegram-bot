...
if __name__ == "__main__":
    try:
        ...
        print("Regi adatok torlese a napi listabol...")
        meccsek_sheet.clear()
        ...
        print(f"API valasz sikeres, {len(matches_today)} meccs talalhato ezen a napon osszesen.")
        
        feldolgozott_meccsek = 0
        print("Szures a megadott ligakra...")
        for match_data in matches_today:
            if match_data['league']['id'] in ERDEKES_LIGAK:
                # ... (a ciklus többi része változatlan)
                ...
                napi_sor = [match_id, fixture['date'], teams['home']['name'], teams['away']['name'], f"{league['name']} ({league['country']})", tip_1x2, tip_goals, tip_btts]
                meccsek_sheet.append_row(napi_sor, value_input_option='USER_ENTERED')
                if tip_1x2 != "N/A": archivum_sheet.append_row([match_id, fixture['date'], meccs_neve, '1X2', tip_1x2, '', 'Függőben'], value_input_option='USER_ENTERED')
                if tip_goals != "N/A (keves adat)": archivum_sheet.append_row([match_id, fixture['date'], meccs_neve, 'Gólok O/U 2.5', tip_goals, '', 'Függőben'], value_input_option='USER_ENTERED')
                if tip_btts != "N/A (keves adat)": archivum_sheet.append_row([match_id, fixture['date'], meccs_neve, 'BTTS', tip_btts, '', 'Függőben'], value_input_option='USER_ENTERED')
                feldolgozott_meccsek += 1
                print(f"Erdekes meccs feldolgozva es mentve: {meccs_neve}")
        
        if feldolgozott_meccsek > 0:
            print(f"Osszesen {feldolgozott_meccsek} meccs feldolgozva es mentve.")
        else:
