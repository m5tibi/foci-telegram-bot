...
if __name__ == "__main__":
    try:
        ...
        supabase.table('meccsek').delete().neq('id', 0).execute() # 'neq' egy trükk, hogy minden sort töröljünk
        ...
        for match_data in matches_today:
            if match_data['league']['id'] in ERDEKES_LIGAK:
                ...
                if tip_1x2 != "N/A": archivumba_sorok.append({'meccs_id': match_id, 'datum': fixture['date'], 'meccs_neve': meccs_neve, 'tipp_tipusa': '1X2', 'tipp_erteke': tip_1x2})
                if tip_goals != "N/A (keves adat)": archivumba_sorok.append({'meccs_id': match_id, 'datum': fixture['date'], 'meccs_neve': meccs_neve, 'tipp_tipusa': 'Gólok O/U 2.5', 'tipp_erteke': tip_goals})
                if tip_btts != "N/A (keves adat)": archivumba_sorok.append({'meccs_id': match_id, 'datum': fixture['date'], 'meccs_neve': meccs_neve, 'tipp_tipusa': 'BTTS', 'tipp_erteke': tip_btts})

                print(f"Erdekes meccs feldolgozva: {meccs_neve}")

        if napi_sorok:
            supabase.table('meccsek').insert(napi_sorok).execute()
            print(f"{len(napi_sorok)} uj sor hozzaadva a 'meccsek' tablahoz.")

        if archivumba_sorok:
            supabase.table('tipp_elo_zmenyek').insert(archivumba_sorok).execute()
            print(f"{len(archivumba_sorok)} tipp hozzaadva az 'archivum' tablahoz.")
        else:
