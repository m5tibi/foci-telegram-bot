# main.py (V5.7 - Manuális Feltöltés Bővítve)
...
async def handle_registration(request: Request, email: str = Form(...), password: str = Form(...)):
    try:
        ...
        supabase.table("felhasnalok").insert({"email": email, "hashed_password": hashed_password, "subscription_status": "inactive"}).execute()
...
# === MÓDOSÍTOTT ADMIN FELTÖLTŐ VÉGPONTOK ===
...
async def handle_upload(request: Request,
    ...
    try:
        ...
        supabase.table("manual_slips").insert({
            ...
            "status": "Folyamatban" # Alapértelmezett státusz
        }).execute()
