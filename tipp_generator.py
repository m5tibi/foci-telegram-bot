# tipp_generator.py (V3.2 - Napi Tuti javítással)
...
def calculate_confidence(tip_type, odds, stats_h, stats_v):
    ...
    if tip_type == "Home" and 1.4 <= odds <= 2.2:
        ...
        if wins_h >= 3 and wins_h > wins_v + 1:
            score += 25; reason.append(f"Hazai csapat jó formában ({form_h}).")
        if float(goals_for_h) > 1.8:
            score += 15; reason.append("Gólerős otthon.")
    elif tip_type == "Away" and 1.4 <= odds <= 2.2:
        score = 60
        if wins_v >= 3 and wins_v > wins_h + 1:
            score += 25; reason.append(f"Vendég csapat jó formában ({form_v}).")
        if float(goals_for_v) > 1.7:
            score += 15; reason.append("Gólerős idegenben.")
        ...
        if avg_goals_total > 3.0:
            score += 40; reason.append(f"Gólerős csapatok (átlagban {avg_goals_total:.1f} gól/meccs).")
        elif avg_goals_total > 2.5:
            score += 25; reason.append("Várhatóan nyílt meccs.")
    elif tip_type == "Mindkét csapat szerez gólt" and 1.5 <= odds <= 2.0:
        score = 50
        ...
        if float(goals_for_h) > 1.2 and float(goals_for_v) > 1.1 and float(conceded_h) > 0.7 and float(conceded_v) > 0.7:
            score += 45; reason.append("Mindkét csapat támadó szellemű és sebezhető.")

    if score > 0 and (odds < 1.5 or odds > 2.2): score -= 10
...
def analyze_and_generate_tips(fixtures):
    ...
    for fixture_data in fixtures:
        fixture = fixture_data.get('fixture', {}); teams = fixture_data.get('teams', {}); league = fixture_data.get('league', {})
        fixture_id = fixture.get('id')
        if not fixture_id: continue
        home_team_id, away_team_id, league_id = teams.get('home', {}).get('id'), teams.get('away', {}).get('id'), league.get('id')
        ...
        for bet in odds_data:
            for value in bet.get('values', []):
                tip_name_map = {"Match Winner.Home": "Home", "Match Winner.Away": "Away", "Both Teams To Score.Yes": "Mindkét csapat szerez gólt", "Over/Under.Over 2.5": "Gólok száma 2.5 felett"}
...
def save_tips_to_supabase(tips):
    ...
    tips_to_insert = [{
        ...
        "liga_nev": tip["liga_nev"], "liga_orszag": tip["liga_orszag"], "tipp": tip["tipp"], "odds": tip["odds"],
        "confidence_score": tip["confidence_score"], "indoklas": tip["indoklas"], "eredmeny": "Tipp leadva"
    } for tip in tips]
...
def create_daily_special(tips):
    ...
    # --- JAVÍTÁS ITT: Először törlünk, hogy tiszta legyen a lap ---
    today_start = datetime.now(BUDAPEST_TZ).replace(hour=0, minute=0, second=0, microsecond=0).astimezone(pytz.utc)
...
def main():
    print(f"Statisztika-alapú Tipp Generátor (V3.2) indítása - {datetime.now(BUDAPEST_TZ)}...")
