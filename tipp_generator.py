# tipp_generator.py (V14.0 - Gemini Intelligens Pontozó Integrációval)
...
# --- Globális Gyorsítótárak (A te eredeti megoldásod) ---
TEAM_STATS_CACHE, STANDINGS_CACHE, H2H_CACHE, INJURIES_CACHE = {}, {}, {}, {}

# --- LIGA PROFILOK (A te eredeti beállításaid alapján) ---
RELEVANT_LEAGUES = {
...
# --- API HÍVÓ FÜGGVÉNY (A te eredeti megoldásod) ---
...
# --- ADATELŐTÖLTŐ FÜGGVÉNYEK (A te eredeti megoldásod) ---
...
# --- ÚJ, INTELLIGENS PONTOZÓ ÉS ELEMZŐ RENDSZER ---
def analyze_and_score_fixture(fixture):
    """
    Ez a központi elemző motor. Lekéri a szükséges adatokat (cache-ből vagy API-ról),
...
# --- SZELVÉNY ÖSSZEÁLLÍTÓ (A te eredeti logikád, de az új elemzővel) ---
def create_doubles_from_tips(today_str, tips):
    all_slips = []
    sorted_tips = sorted(tips, key=lambda x: x['score'], reverse=True)
    
    for combo in combinations(sorted_tips[:6], 2): # Top 6 jelöltből párosítunk
        ...
        if 2.2 <= total_odds <= 4.5:
            ...
            if len(all_slips) >= 3: break # Max 3 szelvény
    return all_slips

# --- ADATBÁZIS MŰVELETEK (A te eredeti megoldásaid) ---
...
def main():
    ...
    if all_potential_tips:
        ...
        if all_slips:
            ...
            record_daily_status(today_str, "Nincs megfelelő tipp", "A jelöltekből nem sikerült szelvényt összeállítani.")
    else:
        record_daily_status(today_str, "Nincs megfelelő tipp", "Egyetlen meccs sem érte el a minimális pontszámot.")
